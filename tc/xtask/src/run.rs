use std::{
    os::unix::process::CommandExt, process::Command,
};
...
use crate::build_ebpf::{
    build_ebpf, Architecture, Options as BuildOptions,
};
...
pub fn run(
    ...
    build(&opts).context(
        "Error while building userspace application",
    )?;
    ...
    // spawn the command
    let err = Command::new(
        args.get(0).expect("No first argument"),
    )
