...
use aya_bpf::{
    macros::socket_filter, programs::SkBuffContext,
};
use core::mem;
use memoffset::offset_of;
use prg_arr_map_common::{
    ICMP_PROG_IDX, ICMP_PROTO, IP_PROTO, TCP_PROG_IDX,
...
fn increment_counter(proto: u8) {
    let mut counter =
        unsafe { COUNTERS.get(&proto).unwrap_or(&0) };
    let new_count = *counter + 1;
    unsafe {
        COUNTERS.insert(&proto, &new_count, 0).unwrap()
    };
}
...
pub fn prg_arr_map(_ctx: SkBuffContext) -> i64 {
    ...
    match ip_proto {
        TCP_PROTO => unsafe {
            JUMP_TABLE.tail_call(&_ctx, TCP_PROG_IDX);
        },
        ICMP_PROTO => unsafe {
            JUMP_TABLE.tail_call(&_ctx, ICMP_PROG_IDX);
        },
        UDP_PROTO => unsafe {
            JUMP_TABLE.tail_call(&_ctx, UDP_PROG_IDX);
        },
        _ => {}
    }
